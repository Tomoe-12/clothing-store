...
if(!empty($result)&& $result->num_rows>0){
 ...
 <img src="data:image/jepg;base64,<?php echo base64_encode($row["per_img"]) ?>" alt="" style="max-height:200px;border-radius:50%;" >
    ...
    <a href="../components/edit.php"  id="edit"style=" width:40%;background-color: rgb(63, 202, 63);color:white;font-size:20px;padding:10px;text-decoration:none;font-weight:bold;border-radius:10px;"> Edit profile </a>
...
<a href="./Lady.php"><div class="cat">Lady's Collections</div></a>
   ...
   $result=$con->query("SELECT * FROM closet where gender='female' order by price");
   if(!empty($result)&& $result->num_rows>0){
    while($row=$result->fetch_assoc()){ 
...
<a href="../components/Detail.php?item_id=<?php  echo $row["clo_id"]; ?>"><div class="div2">View Detail</div></a>
</div>
